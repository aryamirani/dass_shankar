...
export default function EnglishFillBlanks({ onBack }) {
    ...
    return (
        <div className="landing-root" style={{ overflowY: 'auto', display: 'block', height: '100vh', paddingTop: 80 }}>
            {/* Fixed Header */}
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(255,255,255,0.95)', padding: '10px 20px', zIndex: 100, borderBottom: '1px solid #ddd', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <button className="action-btn secondary" onClick={onBack} style={{ padding: '8px 14px', fontSize: 16, minHeight: 40 }}>Back</button>
                <h2 style={{ margin: 0 }}>Fill in the Blanks</h2>
                <button className="action-btn" onClick={checkAnswers} style={{ padding: '8px 20px', fontSize: 16, minHeight: 40, height: 40 }}>Check Answers</button>
            </div>

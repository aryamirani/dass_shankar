...
export default function VocabularyExerciseAd({ onBack, onNextExercise }) {
  // Updated for 'ad' target words
  const initialData = useMemo(() => {
    const distractors = ['an', 'ab', 'ac', 'ap', 'ag', 'at', 'am', 'af', 'ar']
    const countAd = Math.floor(Math.random() * 3) + 3 // 3..5 instances of 'ad'
    const arr = []
    for (let i = 0; i < countAd; i++) arr.push('ad')
    ...
    return { items: arr.map((w, i) => ({ id: i, word: w, matched: false })), countAd }
  }, [])
  ...
  const totalAd = initialData.countAd
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', padding: 20, position: 'relative' }}>
      ...
      <div style={{ width: '100%', maxWidth: 980, padding: 10 }}>
        ...
        <h2 style={{ fontSize: 'clamp(24px, 5vw, 36px)', textAlign: 'center', marginBottom: 8 }}>Find and tap only the "ad" words</h2>
        <div style={{ textAlign: 'center', marginBottom: 12, fontSize: 'clamp(16px, 4vw, 18px)', color: '#333' }}>
          Correct found: {completedCount} / {totalAd}
        </div>

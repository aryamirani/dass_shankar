...
export default function ComputerKeyboard({ onBack, onNextExercise }) {
    ...
    useEffect(() => {
        // Pick a word not yet completed
        const available = wordbank.filter(w => !completed.includes(w))
        if (available.length > 0) {
            setTarget(available[Math.floor(Math.random() * available.length)])
        } else {
            setTarget(null) // All done
        }
    }, [completed])
    ...
    return (
        <div style={{
                ...
                <div style={{
                    ...
                    {target ? (
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '32px', marginBottom: 20 }}>Excellent! You typed all words! 🎉</div>
                            <button
            ...
            <audio ref={correctRef} src="/Grade1/computer/Keyboard/correct.wav" />
            <audio ref={wrongRef} src="/Grade1/computer/Keyboard/wrong.wav" />
        </div>
    )
}
